...
(sb/send-mult-cmd
  ...
  [[:cmd/wire-comp     ; :cmd/wire-comp messages pass a vector of instantiated components
   ...
   [:cmd/route-all {:from :state-cmp :to :ws-cmp}]
   [:cmd/route {:from :ws-cmp :to :state-cmp}]
   [:cmd/route {:from :tweets-cmp :to :state-cmp}]
   [:cmd/route {:from :search-cmp :to :state-cmp}]
   [:cmd/route {:from :sort-cmp :to :state-cmp}]
   [:cmd/route {:from :pag-cmp :to :state-cmp}]
   [:cmd/route {:from :cloud-cmp :to :state-cmp}]
   [:cmd/route {:from :wc-c-cmp :to :state-cmp}]
   [:cmd/route {:from :ws-cmp :to :jvmstats-cmp}]

   [:cmd/observe-state {:from :state-cmp :to :tweets-cmp}]
   [:cmd/observe-state {:from :state-cmp :to :cloud-cmp}]
   [:cmd/observe-state {:from :state-cmp :to :wc-c-cmp}]
   [:cmd/observe-state {:from :state-cmp :to :ts-cmp}]
   [:cmd/observe-state {:from :state-cmp :to :search-cmp}]
   [:cmd/observe-state {:from :state-cmp :to :sort-cmp}]
   [:cmd/observe-state {:from :state-cmp :to :pag-cmp}]
   [:cmd/observe-state {:from :state-cmp :to :count-cmp}]
   [:cmd/observe-state {:from :state-cmp :to :users-count-cmp}]
   [:cmd/observe-state {:from :state-cmp :to :tt-count-cmp}]])
